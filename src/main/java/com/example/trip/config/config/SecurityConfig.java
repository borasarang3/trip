package com.example.trip.config.config;
        ...
        httpSecurity
                ...
                .authorizeHttpRequests(
                        authorization -> authorization
                                .requestMatchers("/tripuser/login/**").permitAll()      //로그인페이지는 누구나 접속이 가능한 권한
...
//                            .anyRequest().authenticated()   //그 이외에는 다 로그인해서 접속해
                ...
                // 로그인
                .formLogin(
                        formLogin ->formLogin.loginPage("/tripuser/login")      //기본 로그인 페이지 지정
                                .defaultSuccessUrl("/")                     //로그인이 성공했다면
                                .usernameParameter("id")                      //로그인 <input name="email">
                        //컨트롤러로 보낼때~~
                )
                // 로그아웃
                .logout(
                        logout -> logout
                                .logoutRequestMatcher(new AntPathRequestMatcher("/tripuser/logout"))           //로그아웃 a태그라 생각
